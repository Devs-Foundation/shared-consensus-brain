...
Un panneau avec des statistiques sur la **machine hébergeant le cerveau** : modèle et cœurs du CPU, charge CPU, RAM, disque utilisé/libre, et une estimation de **croissance du cerveau** basée sur la taille moyenne des notes. Ces relevés sont **locaux et informatifs** — le chiffre de croissance est une estimation approximative, **pas** une promesse de stockage infini. Une mémoire persistante et extensible vient du disque et de git, pas de la magie.

...
一个**100% 本地**的 Markdown/git "大脑"查看窗口：一个可搜索、阅读、编辑和同步的实时图谱 —— 具备自动备份、真实指标和机器状态统计。它是一个**管理与演示工具**，并非用来替代你日常使用的编辑器。
...
一个展示**托管该大脑的机器**统计信息的面板: CPU 型号与核心数、CPU 负载、内存(RAM)、已用/可用磁盘空间,以及基于平均笔记大小估算出的**大脑增长**趋势。这些读数是**本地的、仅供参考的**—— 增长数值只是一个粗略估计,**并非**无限存储空间的承诺。持久、可扩展的记忆来自磁盘和 git,而非凭空而来。
